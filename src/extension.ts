...
export function activate(context: vscode.ExtensionContext) {
  ...
  const getIsCopyPasteAutoConverterEnable = (): boolean => {
    return vscode.workspace
      ...
      .get("isCopyPasteAutoConverterEnable", true);
  };
  ...
  const setIsCopyPasteAutoConverterEnable = async (enabled: boolean): Promise<void> => {
    await vscode.workspace
      .getConfiguration("dto-converter")
      .update("isCopyPasteAutoConverterEnable", enabled, true);
    vscode.window.showInformationMessage(
      `CopyPaste AutoConverter가 ${enabled ? "활성화" : "비활성화"} 되었습니다.`,
    );
  };
  ...
  const getAutoConverterEnabled = (): boolean => {
    return vscode.workspace
      .getConfiguration("dto-converter")
      .get("copyPasteAutoConverter.typeboxToSwift", true);
  };
  ...
  const setAutoConverterEnabled = async (enabled: boolean): Promise<void> => {
    await vscode.workspace
      .getConfiguration("dto-converter")
      .update("copyPasteAutoConverter.typeboxToSwift", enabled, true);
  };
  ...
  const convertTypeboxToSwiftCommand = vscode.commands.registerCommand(
    "dto-converter.convertTypeboxToSwift",
    async () => {
  ...
  // 자동 변환 활성화 명령어
  const enableCopyPasteAutoConverterCommand = vscode.commands.registerCommand(
    "dto-converter.enableCopyPasteAutoConverter",
    async () => {
  ...
  // 자동 변환 비활성화 명령어
  const disableCopyPasteAutoConverterCommand = vscode.commands.registerCommand(
    "dto-converter.disableCopyPasteAutoConverter",
    async () => {
            ...
            .then((success) => {
              if (success) {
                ...
                vscode.window.showErrorMessage(
                  "TypeBox DTO를 Swift 코드로 변환하는 데 실패했습니다",
                );
              }
            });
          return;
