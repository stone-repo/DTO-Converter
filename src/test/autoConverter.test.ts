...
suite('자동 변환 테스트 스위트', () => {
    ...
    setup(async () => {
        ...
        await vscode.workspace.getConfiguration('dto-converter').update('copyPasteAutoConverter.typeboxToSwift', true, true);
    });
    ...
    test('자동 변환 활성화/비활성화 테스트', async () => {
        // 자동 변환 기능 활성화 상태 확인
        let isEnabled = vscode.workspace.getConfiguration('dto-converter').get('isCopyPasteAutoConverterEnable');
        ...
        isEnabled = vscode.workspace.getConfiguration('dto-converter').get('isCopyPasteAutoConverterEnable');
    ...
    test('TypeBox to Swift 변환 설정 테스트', async () => {
        ...
        let isTypeboxToSwiftEnabled = vscode.workspace.getConfiguration('dto-converter').get('copyPasteAutoConverter.typeboxToSwift');
        assert.strictEqual(isTypeboxToSwiftEnabled, true, 'TypeBox to Swift 변환이 기본적으로 활성화되어 있어야 함');
        
        // TypeBox to Swift 변환 비활성화
        await vscode.workspace.getConfiguration('dto-converter').update('copyPasteAutoConverter.typeboxToSwift', false, true);
        isTypeboxToSwiftEnabled = vscode.workspace.getConfiguration('dto-converter').get('copyPasteAutoConverter.typeboxToSwift');
        assert.strictEqual(isTypeboxToSwiftEnabled, false, 'TypeBox to Swift 변환이 비활성화되어야 함');
        
        // TypeBox to Swift 변환 다시 활성화
        await vscode.workspace.getConfiguration('dto-converter').update('copyPasteAutoConverter.typeboxToSwift', true, true);
        isTypeboxToSwiftEnabled = vscode.workspace.getConfiguration('dto-converter').get('copyPasteAutoConverter.typeboxToSwift');
        assert.strictEqual(isTypeboxToSwiftEnabled, true, 'TypeBox to Swift 변환이 다시 활성화되어야 함');
    });
    ...
    test('설정 변경시 클립보드 리스너 활성화/비활성화 테스트', async () => {
        ...
        // 비활성화
        await vscode.workspace.getConfiguration('dto-converter').update('isCopyPasteAutoConverterEnable', false, true);
        const isDisabled = !vscode.workspace.getConfiguration('dto-converter').get('isCopyPasteAutoConverterEnable');
        assert.strictEqual(isDisabled, true, '설정이 비활성화 상태로 변경되어야 함');
        
        // 다시 활성화
        await vscode.workspace.getConfiguration('dto-converter').update('isCopyPasteAutoConverterEnable', true, true);
        const isEnabled = vscode.workspace.getConfiguration('dto-converter').get('isCopyPasteAutoConverterEnable');
