...
suite("Extension Test Suite", () => {
  ...
  test("설정 테스트", async () => {
    // 기본 설정 확인
    const isCopyPasteAutoConverterEnable = vscode.workspace
      .getConfiguration("dto-converter")
      .get("isCopyPasteAutoConverterEnable");
    const typeboxToSwift = vscode.workspace
      .getConfiguration("dto-converter")
      .get("copyPasteAutoConverter.typeboxToSwift");

    // 기본값이 true인지 확인
    assert.strictEqual(
      isCopyPasteAutoConverterEnable,
      true,
      "기본적으로 자동 변환 기능이 활성화되어 있어야 함",
    );
    assert.strictEqual(
      typeboxToSwift,
      true,
      "기본적으로 TypeBox to Swift 변환이 활성화되어 있어야 함",
    );
  });
  ...
  test("자동 변환 설정 변경 테스트", async function () {
    ...
    // 원래 설정값 저장
    const originalAutoConverterSetting = vscode.workspace
      .getConfiguration("dto-converter")
      .get("isCopyPasteAutoConverterEnable");
    ...
    try {
      ...
      let currentSetting = vscode.workspace
        .getConfiguration("dto-converter")
        .get("isCopyPasteAutoConverterEnable");
      ...
      currentSetting = vscode.workspace
        .getConfiguration("dto-converter")
        .get("isCopyPasteAutoConverterEnable");
      ...
      await vscode.workspace
        .getConfiguration("dto-converter")
        .update("isCopyPasteAutoConverterEnable", originalAutoConverterSetting, true);
    }
  });
});
